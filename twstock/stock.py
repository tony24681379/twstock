...
try:
    from .db_utils import to_python_datetime, safe_float
except ImportError:
    from db_utils import to_python_datetime, safe_float
...
class Stock(analytics.Analytics):
    ...
    async def load_data(self, load_data: bool = True, force_reload: bool = False):
        ...
        # 檢查追蹤狀態
        tracker = await self.db_manager.get_tracker(self.sid) if not force_reload else None
        ...
        if force_reload or not tracker or not tracker['daily_loaded']:
            # 從 API 取得並儲存到資料庫
            outstanding_shares = float(self.info_data.outstanding_shares) / 1000 if self.info_data is not None else 1.0
            ...
            # 檢查是否需要增量更新
            needs_update = await self.db_manager.check_needs_update(self.sid, days_threshold=1)
            if needs_update:
                # 增量更新：只取得最近的資料
                outstanding_shares = float(self.info_data.outstanding_shares) / 1000 if self.info_data is not None else 1.0
    ...
    async def load_info_from_db(self) -> pd.Series:
        ...
        async with self.db_manager.get_session() as session:
            result = await session.execute(
                text("""SELECT * FROM stock_info WHERE id = :stock_id"""),
                {"stock_id": self.sid}
            )
            row = result.first()
            if row:
                ...
                eps_result = await session.execute(
                    text("""SELECT year, quarter, eps FROM stock_eps 
        ...
        async with self.db_manager.get_session() as session:
            ...
            for row in result:
                data.append({
                    ...
                    'investment_trust_holding_rate': row.investment_trust_holding_rate or 0,
    ...
    @property
    def date(self):
        if self.daily_data is not None and not self.daily_data.empty and 'date' in self.daily_data.columns:
            return self.daily_data.date.values
        return []
    
    @property
    def volume(self):
        if self.daily_data is not None and not self.daily_data.empty and 'volume' in self.daily_data.columns:
            return self.daily_data.volume.values
        return []
    
    @property
    def high(self):
        if self.daily_data is not None and not self.daily_data.empty and 'high' in self.daily_data.columns:
            return self.daily_data.high.values
        return []
    
    @property
    def low(self):
        if self.daily_data is not None and not self.daily_data.empty and 'low' in self.daily_data.columns:
            return self.daily_data.low.values
        return []
    
    @property
    def open(self):
        if self.daily_data is not None and not self.daily_data.empty and 'open' in self.daily_data.columns:
            return self.daily_data.open.values
        return []
    
    @property
    def close(self):
        if self.daily_data is not None and not self.daily_data.empty and 'close' in self.daily_data.columns:
    ...
    def bollinger_upper(self):
        return self.daily_data.bollinger_upper.values if self.daily_data is not None else []
    
    @property
    def bollinger_lower(self):
        return self.daily_data.bollinger_lower.values if self.daily_data is not None else []
    ...
    def investment_trust(self):
        return self.daily_data.investment_trust.values if self.daily_data is not None else []
    ...
    def foreign_holding_rate(self):
        return self.daily_data.foreign_holding_rate.values if self.daily_data is not None else []
    
    @property
    def investment_trust_holding_rate(self):
        return self.daily_data.investment_trust_holding_rate.values if self.daily_data is not None else []
    
    @property
    def dealer_holding_rate(self):
        return self.daily_data.dealer_holding_rate.values if self.daily_data is not None else []
    
    @property
    def sum_holding_rate(self):
        return self.daily_data.sum_holding_rate.values if self.daily_data is not None else []
    
    @property
    def major_investors(self):
        return self.daily_data.major_investors.values if self.daily_data is not None else []
    ...
    def three_line_diff(self):
        return self.daily_data.three_line_diff.values if self.daily_data is not None else []
    
    @property
    def four_line_diff(self):
        return self.daily_data.four_line_diff.values if self.daily_data is not None else []
    
    @property
    def lending_balance(self):
        return self.daily_data.lending_balance.values if self.daily_data is not None else []
    
    @property
    def borrowing_balance(self):
        return self.daily_data.borrowing_balance.values if self.daily_data is not None else []
