    ...
    def pivot_point(self):
        ...
        for i in range(1, len(self.close) - 2):
            if self.pivots[i] == 1:
                ...
                if len(conter) > 1:
                    diff.append(
                        abs(self.close[conter[-1].index] - self.close[conter[-2].index])
                    )
    ...
    def up_three_line(self):
        for i in range(2, self.length):
            if self.volume[-i + 1] > 1000:
                if (
                    self.change[-i + 1] > 1.5
                    and self.close[-i + 1] > self.open[-i + 1]
                    and self.close[-i + 1] > self.close[-i]
                ):
        ...
        for i in range(2, self.length):
            if self.macdsignal[-i + 1] > 0 and self.macd[-i + 1] > 0:
                if self.change[-i + 1] > 0 and self.close[-i + 1] > self.open[-i + 1]:
                    if (
                        self.macdsignal[-i + 1] > self.macdsignal[-i]
                        and self.macd[-i + 1] > self.macd[-i]
                    ):
            ...
            if self.k9[-i + 1] < 20 and self.d9[-i + 1] < 20:
                if self.change[-i + 1] > 0:
                    if self.k9[-i + 1] > self.k9[-i]:
                        if (
                            self.k9[-i + 1] > self.d9[-i + 1]
                            and self.k9[-i] < self.d9[-i]
                        ):
                            return self.date[-i + 1]
        ...
        if len(self.bollinger_upper) > 10:
            for i in range(2, self.length):
                ...
                if bollinger_dif < 0.07:
                    if (
                        self.volume[-i + 1] > 300
                        and self.volume[-i + 1] > self.volume[-i] * 2
                    ):
    ...
    def long_up(self):
        for i in range(2, self.length):
            if self.change[-i + 1] > 3:
                if (
                    self.close[-i + 1] > self.open[-i + 1]
                    and self.close[-i] < self.open[-i]
                ):
                    if (
                        self.close[-i] > self.open[-i + 1]
    ...
    def up_cross_ma5_ma20(self):
        for i in range(2, self.length):
            if self.ma20[-i] < self.ma20[-i + 1] and self.ma5[-i] < self.ma5[-i + 1]:
                if (
                    self.ma5[-i] < self.ma20[-i]
                    and self.ma5[-i + 1] > self.ma20[-i + 1]
                ):
                    return self.date[-i + 1]
...
class BestFourPoint(object):
    ...
    def best_buy_1(self):
        return (
            self.stock.volume[-1] > self.stock.volume[-2]
            and self.stock.close[-1] > self.stock.open[-1]
        )

    def best_buy_2(self):
        return (
            self.stock.volume[-1] < self.stock.volume[-2]
            and self.stock.close[-1] > self.stock.open[-2]
        )

    def best_buy_3(self):
        return (
            self.stock.continuous(self.stock.moving_average(self.stock.close, 3)) == 1
        )

    def best_buy_4(self):
        return (
            self.stock.moving_average(self.stock.close, 3)[-1]
